...
Prior to performing the association analysis, a Hardy-Weinberg Equilibrium test and an allele frequency study can be performed on the loaded data. The only requirement to do so is to have a gender column on the covariates table, which on this example is true. The information needed to perform those two analysis is the name of the gender column as well as the names used for describing males and females. The name of the gender column has been retrieved before with the use of `ds.colnames("covars")`, the names to describe the gender can be obtained with

```{r}
ds.table1D("covars$gender")$counts
```

On this example is `1/2`. To perform the Hardy-Weinberg Equilibrium test do

```{r}
head(ds.exactHWE("gds.Data", sexcol = "gender", male = "1", female = "2", snpStart = 1000, snpEnd = 2000)[[1]])
```

Please note that on the previous example a SNP window had to be specified to avoid including SNPs from the Y chromosome, as quoting the documentation of the function: "*Y and M and chromsome SNPs are not permitted in the SNP selection, since the HWE test is not valid for these SNPs*".

To get the allele frequencies do 

```{r}
head(ds.alleleFrequency("gds.Data", sexcol = "gender", male = "1", female = "2")[[1]])
```
