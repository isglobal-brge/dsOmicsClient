...
EWAS requires basically the same statistical methods as those used in DGE. It should be notice that the **virtually pooled analysis** we are going to illustrate here can also be performed with transcriptomic data since each study must have different range values. If so, gene expression harmonization should be performed, for instance, by standardizing the data at each study. For EWAS where methylation is measured using beta values (e.g CpG data are in the range 0-1) this is not a problem. In any case, adopting the **federated meta-analysis** approach could be a safe option.
...
This method corresponds to the **virtually pooled analysis** approach and can be very time consiming since the function repeatedly calls the DataSHIELD function `ds.glm()`. We can adopt another strategy that is to run a glm of each feature independently at each study using `limma` package (which is really fast) and then combine the results (i.e. **federated meta-analysis** approach). 
...
This is a genreal method that can be used to meta-analyze p-values We can verify that the results are pretty similar to those obtained using pooled analyses. Here we compute the association for two of the top-CpGs:
...
The results can be visualize using
...
We illustrate how to perform the same GWAS analyses on the asthma example previously analyzed using Bioconductor workflows implemented in `ds.GWAS()` function. It is worth to notice that this workflow and the new R functions implemented in `r Githubpkg("isglobal-brge/dsOmicsClient")` could be used as a guideline to carry out similar analyses using existing analysis tools in genomics such as IMPUTE, SAMtools or BEDtools among many others. 
...
Finally, Manhattan plots can be created by using this generic function that can be addapted to all situations. That is, DGE, EWAS or GWAS from analyses performed at each server of meta-analyzed

```{r manhattaPlink, fig.cap="Manhattan plot for asthma study. The labelled SNP is the one that passed a p-value threshold equal to 1e-6.", fig.height=4}
manhattan(ans.bioC$study1, featureCol = 2, chrCol = 3, posCol=4, pvalCol = 11, 
          threshold = 1e-6)
```

We can see whether inflation is present in our analyses (due to population stratification in GWAS or to hidden variables accouinting for unwanted variability) by computing $\lambda$ value in a QQ-plot





PLINK with two studies ..... TO BE SUPPLIED
