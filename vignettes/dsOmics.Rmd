...
A description of the pre-requisites can be found [here](https://datashield.discourse.group/t/datashield-resources/152). Basically, what is needed is:
...
- OPTIONAL(used to debug): `r Githubpkg("obiba/DSILite")` in the resources branch
...
Notice that the `r Githubpkg("isglobal-brge/dsOmics")` package includes new extensions of the the `r Githubpkg("obiba/resourcer")` package to deal with new types of data such as VFC formats (VCF2GDS). Next subsections further describe what is required along with some examples. 
