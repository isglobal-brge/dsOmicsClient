...
In order to use the functions contained within this package the following R packages must be installed (in their development version):
...
Then, `dsOmicsClient` package allows different types of analyses: pooled and meta-analysis. Both methods are based on fitting different generalized linear models (GLMs) for each feature when assesing association between omic data and the phenotype/trait/condition of interest. 
...
This may help those users who are not familiar with Bioconductor. There are many researchers who have methylation (or gene expression) data in a tabular form. If so, data could be available in the OPAL server as a data.frame and then the following analyses could also be performed. 
...
In order to deal with this type of data, we have extended the resources available at the `r Githubpkg("obiba/resourcer")` package to VCF files. **NOTE**: PLINK files can be translated into VCF files using different pipelines. In R you can use `r Biocpkg("SeqArray")` to get VCF files. 
...
```{r GWAS}
ds.GWAS('gds.Data', model=asthma~age+country)
```
...
## Analysis with PLINK: example of using secure shell programs

GWAS data analysis can also be performed using program that are executed using shell commands. This is the case of [PLINK](http://zzz.bwh.harvard.edu/plink/), one of the state-of-the-art programs to run GWAS and other genomic data analyses such gene-enviroment interactions or polygenic risc score analyses that requires efficient and scalable pipelines. We have allowed the use of SSH service to run programs on a remote server accessible through ssh containing data and analysis tools where R is just used for launching the analyses and aggregating results. Here we illustrate how to perform the same GWAS analyses on the obesity example previously analyzed using `r Githubpkg("isglobal-brge/dsOmicsClient")`. This R code can be used as a guideline to carry out similar analyses using existing analysis tools in genomics such as IMPUTE, SAMtools or BEDtools among many others. 

We start by assigning the resources 

```{r GWAS_shell_1}
library(opalr)
conns <- opal.login(username="administrator", password="password",
                    url="https://opal-test.obiba.org")
opal.assign.resource(conns, "client", "test.obesity_plink")
```

Then we can verify whether the resource is a shell commands resource client object

```{r GWAS_shell_2}
opal.execute(conns, "class(client)")
```

This code lists files in the working folder

```{r GWAS_shell_3}
opal.execute(conns, "client$exec('ls')")
```


Then we assign the base output file name

```{r GWAS_shell_4}
baseout <- opal.execute(conns, "assign('baseout', file.path(tempdir(), 'out'))")
baseout
```

Now, we are ready to run any PLINK command using the `exec` element of our client object. Notice that in this case we want to assess association between the genotype data in bed format and use as phenotype the variable 'obese' that is in the file 'obesity.phe'. The sentence in a PLINK command would be (NOTE: `baseout` encodes for the output folder)

```
plink --bfile obesity --assoc --pheno obesity.phe --pheno-name obese --out baseout --noweb
```

that translates into R

```{r GWAS_shell_gwas}
opal.execute(conns, "client$exec('plink1', c('--bfile', 'obesity', '--assoc', '--pheno', 'obesity.phe', '--pheno-name', 'obese', '--out', baseout, '--noweb'))")
```


This command shows the output files generated 

```{r GWAS_shell_folder}
opal.execute(conns, "list.files(tempdir())")
```


Now, we can retrieve the association p-values (note that this is non-disclosive information) by simply reading the file 'out.assoc'

```{r GWAS_shell_res}
results <- opal.execute(conns, 
                        "read.table(file.path(tempdir(), 'out.assoc'), header=TRUE)")
head(results)
```


```{r GWAS_shell_descr}
opal.execute(conns, "client$exec('plink1', c('--bfile', 'obesity', '--freq', '--hardy', '--pheno', 'obesity.phe', '--pheno-name', 'obese', '--out', baseout, '--noweb'))")

results2 <- opal.execute(conns, 
                        "read.table(file.path(tempdir(), 'out.frq'), header=TRUE)")
head(results2)
```


This close the OPAL session 
