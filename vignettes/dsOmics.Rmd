...
Before performing association analyses, a Hardy-Weinberg Equilibrium test (HWE) and allele frequency estimates can be obtained. Tho this end, a variable providing gender information on the covariates table is required. Remember that we can visualize the names of the variables from our data by executing `ds.colnames("covars")`. In our case, this variable is called "gender", and the levels of this variable are 1 for male and 2 for female as we can see here (NOTE: we cannot use `ds.levels` since gender variable is not a factor): 

```{r tableCovar}
ds.table1D("covars$gender")
```

Therefore, HWE can be performed by:

```{r exactHWE}
ds.exactHWE("gds.Data", sexcol = "gender", male = "1", female = "2", snpStart = 1000, snpEnd = 2000)
```
...
Similarly, allele frequencies estimates can be estimate by:
