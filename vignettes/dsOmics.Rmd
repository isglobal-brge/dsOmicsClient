...
In order to perform omic daa analyses, we need first to login and assigning resources to DataSHIELD
...
Nontheless, as we have previously mentioned, it is recomended to work with the data encapsulated as an `ExpressionSet`. The resource can be assigned to their own R class by using `as.resource.object()` assignment function as following 
...
We can create a QQ-plot by using the generic function `plot` (here not showed). 
...
```{r limma_methy_annot}
ans.limma <- ds.limma(model = ~ diagnosis + Sex,
                      Set = "methy", annotCols = c("CHR", "UCSC_RefGene_Name"),
                      datasources = conns)
ans.limma
```
